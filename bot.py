...
def _run_batch_channel_trigger():
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            print(f"⚠️ batch channel exit {proc.returncode}: {err[:500]}")
    except Exception as e:
        print(f"⚠️ batch channel trigger: {e}")
    finally:
        with _batch_channel_trigger_lock:
            _batch_channel_trigger_running = False
...
def start_batch_channel_listener():
    """User bấm tạo video tự động trên web → bot chạy batch."""
    ...
    print("👂 Lắng nghe batchChannelConfig — tạo video tự động")
